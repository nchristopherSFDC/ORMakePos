  ...
  extends RatingRecord
{
  ...
  public static String CVS_MODULE_INFO = "OpenRate, $RCSfile: CDRRecord.java,v $, $Revision: 1.38 $, $Date: 2011/09/30 17:17:42 $";
  ...
  public void performFieldsValidation(String moduleName)
  {
    ...
    else
    {
      ...
      catch (Exception e)
      {
        ...
        tmpError.setModuleName("InputMap");
        tmpError.setErrorDescription("Invalid start date:"+fields[IDX_START_DATE]);
        this.addError(tmpError);
      }
    }
    if (fields[IDX_ENTRY_DATE] == null || fields[IDX_ENTRY_DATE].equalsIgnoreCase(""))
    {
      RecordError tmpError = new RecordError("ERR_ENTRY_DATE_MISSING",ErrorType.DATA_VALIDATION);
      tmpError.setModuleName("InputMap");
      tmpError.setErrorDescription("Missing Entry date");
      this.addError(tmpError);
    }
    else
    {
      ...
      {
        SimpleDateFormat sdf = new SimpleDateFormat(DATE_FORMAT);
        this.EntryDateUsed = sdf.parse(fields[IDX_ENTRY_DATE]);
      }
      catch (Exception e)
      {
        RecordError tmpError = new RecordError("ERR_ENTRY_DATE_INVALID",ErrorType.DATA_VALIDATION);
        tmpError.setModuleName("InputMap");
        tmpError.setErrorDescription("Invalid Entry date:"+fields[IDX_ENTRY_DATE]);
        this.addError(tmpError);
      }
    }
